...
<head>
    ...
    <title>Favorit Saya - Wisata Indonesia</title>
...
<body class="font-sans antialiased" x-data="{ open: false }">
    ...
    <nav class="bg-white shadow-lg sticky top-0 z-50">
        <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
            <div class="flex justify-between h-16">
                ...
                <div class="hidden sm:flex sm:items-center sm:space-x-8">
                    ...
                    <a href="{{ route('favorites.index') }}" class="text-blue-600 border-b-2 border-blue-600 px-3 py-2 text-sm font-medium">Favorit</a>
                    @auth
                        ...
                        @else
                            <a href="{{ route('profile.show') }}" class="text-gray-700 hover:text-blue-600 px-3 py-2 text-sm font-medium">Profil</a>
                        @endif
                    @endauth
                </div>
        ...
        <div :class="{'block': open, 'hidden': !open}" class="hidden sm:hidden">
            <div class="px-2 pt-2 pb-3 space-y-1">
                ...
                <a href="{{ route('favorites.index') }}" class="block px-3 py-2 text-base font-medium text-blue-600 bg-blue-50 rounded-md">Favorit</a>
                @auth
                    @if(auth()->user()->role === 'admin')
    ...
    <div class="bg-gradient-to-r from-blue-600 to-blue-800 text-white py-12">
        <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
            <h1 class="text-4xl font-bold mb-4">❤️ Destinasi Favorit Saya</h1>
            <p class="text-xl text-blue-100">Simpan dan akses destinasi wisata favorit Anda</p>
        </div>
    </div>
    ...
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-12">
        @if($favorites->count() > 0)
            <div class="mb-6">
                <p class="text-gray-600">
                    Anda memiliki <span class="font-semibold text-blue-600">{{ $favorites->total() }}</span> destinasi favorit
                </p>
            </div>
                        ...
                        <div class="p-6">
                            <div class="flex items-center justify-between mb-2">
                                ...
                                <div class="flex items-center">
                                    ...
                                    <span class="ml-1 text-sm text-gray-600">{{ $destination->getAverageRating() }}</span>
                                    <span class="ml-1 text-sm text-gray-400">({{ $destination->getReviewCount() }})</span>
                                </div>
                            </div>
                            <h3 class="text-xl font-bold text-gray-800 mb-2">{{ $destination->name_232136 }}</h3>
                            <p class="text-gray-600 text-sm mb-4">📍 {{ $destination->location_232136 }}</p>
                            <p class="text-gray-600 line-clamp-2">{{ Str::limit($destination->description_232136, 100) }}</p>
                        </div>
                    </a>
                    
                    <!-- Remove from favorites button -->
                    <button 
                        @click="removeFavorite({{ $destination->id }})"
                        x-data="{ 
                            removeFavorite(id) {
                        ...
                        class="absolute top-4 right-4 bg-white p-2 rounded-full shadow-lg hover:bg-red-50 transition">
        ...
        @else
            <div class="text-center py-12">
                ...
                <p class="text-gray-500 mb-6">Anda belum menyimpan destinasi favorit apapun</p>
                <a href="{{ route('public.destinations') }}" class="inline-block bg-blue-600 text-white px-8 py-3 rounded-lg font-semibold hover:bg-blue-700 transition">
                    Jelajahi Destinasi
                </a>
            </div>
        @endif
            ...
            <div class="grid grid-cols-1 md:grid-cols-3 gap-8">
                ...
                <div>
                    ...
                    <ul class="space-y-2">
                        ...
                        <li><a href="{{ route('favorites.index') }}" class="text-gray-400 hover:text-white">Favorit</a></li>
                    </ul>
                </div>
                <div>
